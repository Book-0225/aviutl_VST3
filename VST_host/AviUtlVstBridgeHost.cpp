...
public:
    VstHost(HINSTANCE hInstance, uint64_t unique_id) :
        ...
        m_hGuiWindow(NULL), m_component(nullptr), m_processor(nullptr),
        m_controller(nullptr), m_plugView(nullptr), m_windowController(nullptr),
        m_mainLoopRunning(false), m_threadsRunning(false),
    ...
    void Cleanup() {
        ...
        if (m_hPipeThread) WaitForSingleObject(m_hPipeThread, 2000);
        if (m_hAudioThread) WaitForSingleObject(m_hAudioThread, 2000);

        if (m_hPipeThread) CloseHandle(m_hPipeThread);
        if (m_hAudioThread) CloseHandle(m_hAudioThread);

        ReleasePlugin();

        if (m_pSharedMem) UnmapViewOfFile(m_pSharedMem);
        if (m_hShm) CloseHandle(m_hShm);
        if (m_hEventClientReady) CloseHandle(m_hEventClientReady);
        if (m_hEventHostDone) CloseHandle(m_hEventHostDone);
...
private:
    ...
    static LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
        VstHost* host = (VstHost*)GetWindowLongPtr(hWnd, GWLP_USERDATA);
        if (message == WM_CREATE) {
            host = (VstHost*)((CREATESTRUCT*)lParam)->lpCreateParams;
        ...
        if (host) {
            ...
            case WM_DESTROY: host->OnGuiClose(); SetWindowLongPtr(hWnd, GWLP_USERDATA, NULL); break;
            }
        }
                    ...
                    if (full_data.rfind("VST3_DUAL:", 0) == 0) {
                        ...
                        if (!decoded_bytes.empty()) {
                            ...
                            while (start_pos < manual_state.length()) {
                                size_t eq_pos = manual_state.find('=', start_pos);
                                size_t sc_pos = manual_state.find(';', start_pos);
                                ...
                                try {
                                    ...
                                    ParamID id = std::stoul(id_str, nullptr, 16);
                                    ParamValue value = std::stod(val_str);
    ...
    bool LoadPlugin(const std::string& path, double sampleRate, int32 blockSize) {
        ...
        if (!m_module) { DbgPrint(_T("LoadPlugin: Error loading module: %hs"), error.c_str()); return false; }

        auto factory = m_module->getFactory();
        ClassInfo audioProcessorClass;
        bool found = false;
        for (const auto& classInfo : factory.classInfos()) {
            if (classInfo.category() == kVstAudioEffectClass) { audioProcessorClass = classInfo; found = true; break; }
        }
        ...
        if (!m_processor) { DbgPrint(_T("LoadPlugin: Failed to get IAudioProcessor.")); ReleasePlugin(); return false; }

        m_processor->setProcessing(false);
        ProcessSetup setup{ kRealtime, kSample32, blockSize, sampleRate };
        if (m_processor->setupProcessing(setup) != kResultOk) { DbgPrint(_T("LoadPlugin: setupProcessing failed.")); ReleasePlugin(); return false; }

        int32 numIn = m_component->getBusCount(kAudio, kInput), numOut = m_component->getBusCount(kAudio, kOutput);
    ...
    void ReleasePlugin() {
        ...
        if (m_processor) { m_processor->setProcessing(false); m_processor->release(); m_processor = nullptr; }
        ...
        if (m_plugProvider) m_plugProvider->releasePlugIn(m_component, m_controller);
...
private:
    ...
    Module::Ptr m_module;
    std::shared_ptr<PlugProvider> m_plugProvider;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    try {
        if (lpCmdLine && *lpCmdLine) {
            ...
            DbgPrint(_T("WinMain: Parsed unique_id from command line: %llu"), unique_id);
        }
    }
    catch (...) {
        unique_id = GetCurrentProcessId();
        DbgPrint(_T("WinMain: Failed to parse unique_id, using PID as fallback: %llu"), unique_id);
    }

    FUnknownPtr<IHostApplication> context(MyHostApp::createInstance());
    ...
    if (g_pVstHost) { delete g_pVstHost; g_pVstHost = nullptr; }
